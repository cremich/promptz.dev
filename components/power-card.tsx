...
export function PowerCard({ power, className }: PowerCardProps) {
  ...
  // Generate SEO-friendly slug for the detail page
  const powerSlug = idToSlug(power.id)

  return (
    <Link href={`/powers/${powerSlug}`} className="block">
      <Card className={cn("transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-900", className)}>
        <CardHeader>
